...
el.innerHTML = list.map((a)=>{ const i=ARTICLES.indexOf(a); return ` <div onclick="openArticle(${i})" style="background:var(--dark2);border:1px solid var(--border);border-radius:var(--r2);padding:18px;transition:all .2s;cursor:pointer;" onmouseover="this.style.borderColor='var(--border2)';this.style.transform='translateY(-2px)'" onmouseout="this.style.borderColor='var(--border)';this.style.transform=''"> <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;gap:8px;flex-wrap:wrap;"> <span style="font-size:9px;font-weight:700;letter-spacing:1px;padding:3px 8px;border-radius:4px;background:rgba(201,168,76,.1);color:var(--gold);">${a.tag}</span> ${a.locked?'<span style="font-size:10px;color:var(--muted);">🔒 Wynnr</span>':'<span style="font-size:10px;color:var(--green2);">● Free</span>'} </div> <div style="font-size:14px;font-weight:600;margin-bottom:6px;line-height:1.4;">${a.title}</div> <div style="font-size:11px;color:var(--muted);margin-bottom:10px;">${a.time}</div> <div style="font-size:12px;color:var(--gold);font-weight:600;">Read article →</div> </div>`; }).join(’’);
}
...
if(!el) return;
el.innerHTML = HC_PARLAYS.map(p=>` <div style="background:var(--dark2);border:1px solid rgba(201,168,76,.18);border-radius:var(--r2);padding:20px;position:relative;overflow:hidden;"> <div style="position:absolute;top:0;left:0;right:0;height:2px;background:linear-gradient(90deg,var(--gold),var(--gold2));"></div> <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:12px;flex-wrap:wrap;gap:8px;"> <div> <div style="font-size:10px;font-weight:700;letter-spacing:1px;color:var(--gold);margin-bottom:4px;">💎 HIGH CONFIDENCE PARLAY</div> <div style="font-family:var(--fd);font-size:22px;letter-spacing:.5px;">${p.title}</div> </div> <div style="text-align:right;"> <div style="font-family:var(--fm);font-size:20px;color:var(--green2);">${p.combinedOdds}</div> <div style="font-size:10px;color:var(--muted);">Combined odds</div> </div> </div> <div style="display:flex;flex-direction:column;gap:8px;margin-bottom:14px;"> ${(p.legs||[]).map((leg,i)=>`<div style="background:var(--dark3);border-radius:8px;padding:10px 14px;display:flex;justify-content:space-between;align-items:flex-start;gap:8px;">
<div>
...
</div>`).join('')} </div> <div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:12px;"> <div style="background:var(--dark3);border-radius:7px;padding:8px 12px;text-align:center;flex:1;min-width:70px;"> <div style="font-family:var(--fd);font-size:18px;color:var(--parch);">${p.winProb}</div> <div style="font-size:9px;color:var(--muted);text-transform:uppercase;letter-spacing:.5px;">Win prob</div> </div> <div style="background:var(--dark3);border-radius:7px;padding:8px 12px;text-align:center;flex:1;min-width:70px;"> <div style="font-family:var(--fd);font-size:18px;color:var(--green2);">${p.ev}</div> <div style="font-size:9px;color:var(--muted);text-transform:uppercase;letter-spacing:.5px;">EV</div> </div> <div style="background:var(--dark3);border-radius:7px;padding:8px 12px;text-align:center;flex:1;min-width:70px;"> <div style="font-family:var(--fd);font-size:18px;color:var(--gold);">${p.units}</div> <div style="font-size:9px;color:var(--muted);text-transform:uppercase;letter-spacing:.5px;">Bet size</div> </div> <div style="background:var(--dark3);border-radius:7px;padding:8px 12px;text-align:center;flex:1;min-width:80px;"> <div style="font-size:11px;font-weight:700;color:${p.ratingColor};margin-top:2px;">${p.rating}</div> <div style="font-size:9px;color:var(--muted);text-transform:uppercase;letter-spacing:.5px;">Rating</div> </div> </div> <div style="font-size:11px;color:var(--muted2);font-style:italic;padding-top:10px;border-top:1px solid var(--border);">Remember: parlays are bonus plays only. Each leg should have standalone edge. Max 1 unit per parlay.</div> </div>`).join(’’);
}
...
if(!el)return;
el.innerHTML=BI_TIERS.map(t=>`<div style="display:flex;align-items:flex-start;gap:14px;padding:14px;background:var(--dark2);border:1px solid var(--border);border-radius:var(--r2);margin-bottom:8px;transition:border-color .2s;" onmouseover="this.style.borderColor='var(--border2)'" onmouseout="this.style.borderColor='var(--border)'"> <div style="width:50px;height:50px;border-radius:9px;display:flex;align-items:center;justify-content:center;font-size:8.5px;font-weight:800;letter-spacing:.4px;text-align:center;line-height:1.4;flex-shrink:0;background:${t.bg};color:${t.color};border:1px solid ${t.border};">${t.badge}</div> <div style="flex:1;"> <div style="font-size:14px;font-weight:600;color:${t.color};margin-bottom:3px;">${t.name}</div> <div style="font-size:12px;color:var(--muted2);line-height:1.65;margin-bottom:7px;">${t.desc}</div> <div style="display:flex;gap:14px;flex-wrap:wrap;">${(t.stats||[]).map(([l,v])=>`<span style="font-size:10px;color:var(--muted);">${l}: <b style="color:var(--parch);">${v}</b></span>`).join(’’)}</div>
</div>
...
out += ‘<a href="'+o.url+'" target="_blank" rel="noopener" style="display:flex;align-items:center;justify-content:center;background:var(--gold);color:#000;border-radius:var(--r);padding:11px 20px;font-weight:700;font-size:13px;text-decoration:none;">’+o.cta+’ →</a>’;
...
return`<div class="trend-card rise"> <div class="trend-header"> <div class="trend-title">${t.title}</div> <div style="text-align:right;"><div class="trend-record ${t.color}">${t.record}</div><div class="trend-pct tp-${t.color}">${pct}%</div></div> </div> <div class="trend-bar"><div class="trend-fill" style="width:${pct}%;background:${fillColor};"></div></div> <div class="trend-desc">${t.desc}</div> <div class="trend-sample">${t.sample}</div> </div>`;
}).join(’’);
...
var *key = (currentBook||‘dk’)+’*’+(document.getElementById(‘sportSel’)?.value||‘ufc’);
...
var poolKy=(currentBook||‘dk’)+’*’+(document.getElementById(‘sportSel’)?.value||‘ufc’);
var pnk=p.name.replace(/[^a-z0-9]/gi,’*’);
...
const pKey = p.name.replace(/[^a-z0-9]/gi,’*’);
const minExp = parseFloat(poolPrefsState[’minexp*’+pKey] || ‘0’) || 0;
return minExp >= 100;
});
...
var pKey=p.name.replace(/[^a-z0-9]/gi,’*’);
var minE=parseFloat((poolState_bp[poolKey_bp]||{})[’minexp*’+pKey]||‘0’)||0;
return minE>=100;
});
...
return `<div class="qa-item"> <div> <div class="qa-title">${i.title}</div> <div class="qa-desc">${i.desc}</div> </div> <div class="qa-actions"> <span class="qa-pill ${chipClass}">${st.toUpperCase()}</span> <button class="btn btn-dark btn-xs" onclick="markQa('${i.id}','pass')">Pass</button> <button class="btn btn-outline btn-xs" onclick="markQa('${i.id}','fail')">Fail</button> <button class="btn btn-outline btn-xs" onclick="markQa('${i.id}','pending')">Pending</button> </div> </div>`;
}).join(’’);
}
...
a.href=url;
a.download=‘onlywynnrs_’+book+’*’+sport+’*’+new Date().toISOString().slice(0,10)+’.csv’;
