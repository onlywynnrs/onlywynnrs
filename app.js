...
function buildPortfolio(){
  ...
  total=selected.reduce(function(s,p){return s+p.salary;},0);if(selected.length!==SIZE||(new Set(selected.map(function(p){return p.name;}))).size!==SIZE||total>CAP||(salMin>0&&total<salMin))continue;if(uniqPct>0&&lineups.length>0){var sN=selected.map(function(p){return p.name;});if(lineups.some(function(ex){var eN=ex.map(function(p){return p.name;});var sh=sN.filter(function(n){return eN.indexOf(n)>-1;}).length;return Math.round(((SIZE-sh)/SIZE)*100)<uniqPct;}))continue;}
...
async function creditReferrer(refCode){var res=await _sbFetch('/rest/v1/profiles?ref_code=eq.'+refCode+'&select=id,ref_credit,ref_count');if(!res.ok||!res.data||!res.data.length)return;var r=res.data[0];var newCredit=(parseFloat(r.ref_credit)||0)+5.80;var newCount=(parseInt(r.ref_count)||0)+1;await _sbFetch('/rest/v1/profiles?id=eq.'+r.id,{method:'PATCH',headers:{'Prefer':'return=minimal','Content-Type':'application/json'},body:JSON.stringify({ref_credit:newCredit,ref_count:newCount,updated_at:new Date().toISOString()})});}
...
function renderAmbassadorDash(containerId){
  ...
  if(!stats.code){
    el.innerHTML='<div style="padding:12px;text-align:center;color:var(--muted2);font-size:12px;">Generating your referral code...</div>';
    ensureRefCode().then(function(){if(_profile)renderAmbassadorDash(containerId);});
    return;
  }
  ...
  var html='<div style="margin-top:16px;padding-top:16px;border-top:1px solid var(--border);">'
    ...
    +'</div>';
  el.innerHTML=html;
  var cb=document.getElementById('amb-copy-code');
  if(cb)cb.onclick=function(){navigator.clipboard.writeText(this.dataset.val);var t=this;t.textContent='Copied!';setTimeout(function(){t.textContent='Copy';},2000);};
  var lb=document.getElementById('amb-copy-link');
  if(lb)lb.onclick=function(){navigator.clipboard.writeText(this.dataset.val);var t=this;t.textContent='Copied!';setTimeout(function(){t.textContent='Copy';},2000);};
}
