...
function buildFullPicks(filter){
  ...
  if(!el) return;
  var _p=window.PICKS||PICKS||[];
  var allPicks = filter==='all' ? _p : _p.filter(function(p){return p.sport===filter;});
...
async function loadDailyContent(){
  try{
    ...
    if(!res.ok||!res.data||!res.data.length){console.log('[OW] No Supabase content for '+today+', using data.js');return;}
    var c=res.data[0];
...
async function loadDailyContent(){try{var today=new Date().toISOString().split('T')[0];var res=await _sbFetch('/rest/v1/daily_content?date=eq.'+today+'&select=*&limit=1');if(!res.ok||!res.data||!res.data.length){console.log('[OW] No Supabase content for '+today+', using data.js');return;}var c=res.data[0];var parse=function(f){try{return f?JSON.parse(f):null;}catch(e){return null;}};var picks=parse(c.picks),signals=parse(c.sharp_signals),parlays=parse(c.parlays),articles=parse(c.articles),ticker=parse(c.ticker),fmPicks=parse(c.fm_picks),oddsBoard=parse(c.odds_board),lvData=parse(c.lv_data);if(picks&&picks.length)window.PICKS=picks;if(signals&&signals.length)window.SHARP_DATA=signals;if(parlays&&parlays.length)window.HC_PARLAYS=parlays;if(articles&&articles.length)window.ARTICLES=articles;if(ticker&&ticker.length)window.TICKER_DATA=ticker;if(fmPicks&&fmPicks.length)window.FM_PICKS=fmPicks;if(oddsBoard)window.ODDS_DATA=oddsBoard;if(lvData&&lvData.length)window.LV_DATA=lvData;console.log('[OW] Live content loaded from Supabase: '+today+' picks='+((window.PICKS||[]).length)+' signals='+((window.SHARP_DATA||[]).length));}catch(err){console.log('[OW] Content load error:',err.message);}}
